...
export class RecibosComponent {
  ...
  tareas: any[] = JSON.parse(localStorage.getItem('_mis_trabajos_') || JSON.stringify([
  ...
  async generarPDF() {
    ...
    pdf.add(
      new Txt('Desarrollo de Sistema Integral Operativo (SIO), incluyendo Backend, Frontend, estructuración de Base de Datos y soporte técnico.')
        .italics().color('#636e72').end
    );
    ...
    pdf.add(
      ...
      ]).fontSize(9).margin([0, 0, 0, 10]).end
    );
