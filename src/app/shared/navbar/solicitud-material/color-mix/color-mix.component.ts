...
export class ColorMixComponent implements OnInit {
  ...
  guardarSolicitud(i) {
    ...
    this.api.NuevaSolicitud({
      cantidad: this.cantidad_a_preparar,
      material: this.material_id,
      tag: 'Preparacion',
      materiales: data,
      motivo: `Preparación ${this.cantidad_a_preparar}kg de ${this.name}`,
    });
    setTimeout(() => {
      this.onCloseModal.emit();
      Swal.fire({
  ...
  ImprimirEtiquetas(i: number) {
    ...
    for (let i = 0; i < requi.materiales.length; i++) {
      if (requi.materiales[i].material.grupo === '66bf583c605b8ca0df5680d3') {
        data.formula[i] =
          `${requi.materiales[i].cantidad} Kg ${requi.materiales[i].material.nombre} ${requi.materiales[i].material.serie}`;
      }
