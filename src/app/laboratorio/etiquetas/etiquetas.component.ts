...
export class EtiquetasComponent {
  ...
  descargarPDF = async () => {
    ...
    pdf.add(
      ...
      ]).widths(['39.5%', '1%', '59.5%']).layout('noBorders').alignment('center')
        .layout({
          hLineWidth: () => 0.25,
          vLineWidth: () => 0,
          hLineColor: () => 'black'
    ...
    pdf.add(
      new Table([
        [
          ...
          new Cell(new Txt(this.formatFecha(this.recepcion.materiales[0][0].fabricacion)).margin([0, 3]).end).fontSize(10).end
        ],
      ]).widths(['47%', '53%'])
        .layout({
          hLineWidth: () => 0.25,
    ...
    pdfDoc.getBase64((base64: string) => {
      this.http.post('https:192.168.0.22/api/print', {
        pdfBase64: base64,
