...
export class AnalisisService {
  ...
  BusquedaDeTodosLosAnalisis() {
    ...
    const currentMonth = new Date().getMonth() + 1; // Los meses en JS son 0-indexados
  ...
  calcularAnualYMensual() {
    this.AnalisisEnElAno =
      this.TintaAnalizadaEnElAno +
      this.SustratoAnalizadaEnElAno +
      (this.CajaAnalizadaEnElAno + this.PadsAnalizadaEnElAno) +
      this.OtrosAnalizadaEnElAno;
    this.AnalisisEnElMes =
      this.TintaAnalizadaEnElMes +
      this.SustratoAnalizadaEnElMes +
      (this.CajaAnalizadaEnElMes + this.PadsAnalizadaEnElMes) +
      this.OtrosAnalizadaEnElMes;
  }
  ...
  BuscarAnalisisTinta() {
    ...
    this.socket.io.on('SERVER:AnalisisTinta', async (AnalisisTinta) => {
      ...
      const aprobados = this.AnalisisTintas.filter(
        (analisis) =>
          analisis.resultado.resultado === 'APROBADO' && new Date(analisis.createdAt).getFullYear() === year,
      );

      const rechazados = this.AnalisisTintas.filter(
        (analisis) =>
    ...
    this.socket.io.on('SERVER:AnalisisSustrato', async (AnalisisSustrato) => {
      ...
      // Filtrar los AnalisisTintas con resultado APROBADOS y RECHAZADOS en el año actual
      const aprobados = this.AnalisisSustrato.filter(
        (analisis) =>
          analisis.resultado.resultado === 'APROBADO' && new Date(analisis.createdAt).getFullYear() === year,
      );

      const rechazados = this.AnalisisSustrato.filter(
        (analisis) =>
          analisis.resultado.resultado === 'RECHAZADO' && new Date(analisis.createdAt).getFullYear() === year,
      );
    ...
    this.socket.io.on('SERVER:AnalisisCajas', async (AnalisisCajas) => {
      ...
      // Filtrar los AnalisisTintas con resultado APROBADOS y RECHAZADOS en el año actual
      const aprobados = this.AnalisisCajas.filter(
        (analisis) =>
          analisis.resultado.resultado === 'APROBADO' && new Date(analisis.createdAt).getFullYear() === year,
      );

      const rechazados = this.AnalisisCajas.filter(
        (analisis) =>
          analisis.resultado.resultado === 'RECHAZADO' && new Date(analisis.createdAt).getFullYear() === year,
      );
    ...
    this.socket.io.on('SERVER:AnalisisPads', async (AnalisisPads) => {
      ...
      // Filtrar los AnalisisTintas con resultado APROBADOS y RECHAZADOS en el año actual
      const aprobados = this.AnalisisPads.filter(
        (analisis) =>
          analisis.resultado.resultado === 'APROBADO' && new Date(analisis.createdAt).getFullYear() === year,
      );

      const rechazados = this.AnalisisPads.filter(
        (analisis) =>
          analisis.resultado.resultado === 'RECHAZADO' && new Date(analisis.createdAt).getFullYear() === year,
      );
    ...
    this.socket.io.on('SERVER:AnalisisOtros', async (AnalisisOtros) => {
      ...
      // Filtrar los AnalisisTintas con resultado APROBADOS y RECHAZADOS en el año actual
      const aprobados = this.AnalisisOtros.filter(
        (analisis) =>
          analisis.resultado.resultado === 'APROBADO' && new Date(analisis.createdAt).getFullYear() === year,
      );

      const rechazados = this.AnalisisOtros.filter(
        (analisis) =>
          analisis.resultado.resultado === 'RECHAZADO' && new Date(analisis.createdAt).getFullYear() === year,
      );
